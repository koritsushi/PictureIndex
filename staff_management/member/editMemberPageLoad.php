    ...
    <?php if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) {
            if ($row["memberPics"] == "") {
                ...
                echo '<img id="blah" src="http://localhost/PictureIndex/staff_management/member' . $row["memberPics"] . '" alt="' . $row["memberPics"] . '" class="imgBox" />';
            }
        }
    } ?>
...
<div class="col-9 row">
    ...
    <div class="form-inline col-6 row">
        <label for="memberName" class="mb-2 mr-sm-2 col-4">Member Name :</label>
        <?php if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) { ?>
